...
#define rec_yaw				0			//Transmitter Channel 1
#define rec_pitch			1			//Transmitter Channel 2
#define rec_throttle		2			//Transmitter Channel 3
#define rec_roll			3			//Transmitter Channel 4
...
#define P_AnglePitch	2.0f
#define I_AnglePitch	0.3f
#define D_AnglePitch	0.7f

#define P_AngleRoll		P_AnglePitch
#define I_AngleRoll		I_AnglePitch
#define D_AngleRoll		D_AnglePitch
...
float desired_angle_pitch 	 = 0, desired_angle_roll 	 = 0;
float error_angle_pitch 	 = 0, error_angle_roll 		 = 0;
float error_angle_pitch_prev = 0, error_angle_roll_prev  = 0;
float angle_pitch_iterm_prev = 0, angle_roll_iterm_prev  = 0;

uint32_t loopTimer = 0;

float PID_Return[2] = {0, 0};			//Prev_Error, I_Term

int pid_update(float error, float P, float I, float D, float previous_error, float previous_i)
{
...
void pid_reset(void)
{
	...
	error_angle_pitch_prev = 0;
	error_iterm_pitch_prev = 0;
	error_angle_roll_prev = 0;
	error_iterm_roll_prev = 0;
}

///////////////////////////////////// PID SECTION END /////////////////////////////////////

///////////////////////////////////// KALMAN FILTER SECTION START /////////////////////////////////////

float angle_kalman_pitch_prev = 0, uncert_angle_kalman_pitch_prev = 0;
float angle_kalman_roll_prev  = 0, uncert_angle_kalman_roll_prev  = 0;

float Kalman_Return[2] = {0, 0};			//State_Prev, Uncert_Prev

float kalman_update(float kalman_angle_prev, float uncert_kalman_angle_prev, float gyro_rate, float angle_acc)
{
	//Prediction for Angle using Gyro Rate
	float angle_kalman = kalman_angle_prev + (gyro_rate * time_period);

	//Uncertainity of Current Angle Prediction
	float uncert_angle_kalman = uncert_kalman_angle_prev + ((4 * 4) * (time_period * time_period));

	//Calculation of Kalman Gain
	float kalman_gain = uncert_angle_kalman / (uncert_angle_kalman + (3 * 3));

	//Current Angle Calculation with Accelerometer Angle
	angle_kalman = angle_kalman + (kalman_gain * (angle_acc - angle_kalman));

	//Set Current Angle to Previous Angle for next iteration
	Kalman_Return[0] = angle_kalman;
	
	//Update Previous Uncertainity
	Kalman_Return[1] = (1 - kalman_gain) * uncert_angle_kalman;

	return angle_kalman;
}

///////////////////////////////////// KALMAN FILTER SECTION END /////////////////////////////////////
...
int main(void)
{
	...
	while(1)
    {
		...
		if(armed == 1)						//The PID Control Loop
		{
			...
			if(rec_ch[rec_throttle] <= 1140) pid_reset();
			...
			//Calculate the Current Accelerometer Measurements in X, Y, Z Axis
			float acc_x = ((float)imu.accel_raw_x / (float)ACCEL_SENS_SCALE_FACTOR) + ACCEL_ERROR_X;
			float acc_y = ((float)imu.accel_raw_y / (float)ACCEL_SENS_SCALE_FACTOR) + ACCEL_ERROR_Y;
			float acc_z = ((float)imu.accel_raw_z / (float)ACCEL_SENS_SCALE_FACTOR) + ACCEL_ERROR_Z;

			float angle_pitch = (atan(-acc_x / sqrt((acc_y * acc_y) + (acc_z * acc_z))) * 180.0f) / 3.14f;
			float angle_roll  = (atan(acc_y  / sqrt((acc_x * acc_x) + (acc_z * acc_z))) * 180.0f) / 3.14f;

			//Kalman Filter Angle Pitch
			float angle_kalman_pitch = kalman_update(angle_kalman_pitch_prev, uncert_angle_kalman_pitch_prev, rate_pitch, angle_pitch);

			angle_kalman_pitch_prev = Kalman_Return[0];
			uncert_angle_kalman_pitch_prev = Kalman_Return[1];

			//Kalman Filter Angle Roll
			float angle_kalman_roll = kalman_update(angle_kalman_roll_prev, uncert_angle_kalman_roll_prev, rate_roll, angle_roll);

			angle_kalman_roll_prev = Kalman_Return[0];
			uncert_angle_kalman_roll_prev = Kalman_Return[1];
			
			//Read and Convert Receiver Values
			desired_angle_pitch = 0.08 * (rec_ch[rec_pitch] - 1500);
			desired_angle_roll  = 0.08 * (rec_ch[rec_roll]  - 1500);
			inputThrottle = rec_ch[rec_throttle];
			desired_rate_yaw   = 0.15 * (rec_ch[rec_yaw]   - 1500);

			//Calculate errors for pitch and roll angles
			error_angle_pitch = desired_angle_pitch - angle_kalman_pitch;
			error_angle_roll  = desired_angle_roll  - angle_kalman_roll;
			
			//Update desired_rate_pitch with PID
			desired_rate_pitch = pid_update(error_angle_pitch, P_AnglePitch, I_AnglePitch, D_AnglePitch, error_angle_pitch_prev, angle_pitch_iterm_prev);

			error_angle_pitch_prev = PID_Return[0];
			angle_pitch_iterm_prev = PID_Return[1];
			
			//Update desired_rate_roll with PID
			desired_rate_roll = pid_update(error_angle_roll, P_AngleRoll, I_AngleRoll, D_AngleRoll, error_angle_roll_prev, angle_roll_iterm_prev);

			error_angle_roll_prev = PID_Return[0];
			angle_roll_iterm_prev = PID_Return[1];
			...
			inputPitch = pid_update(error_rate_pitch, P_Pitch, I_Pitch, D_Pitch, error_rate_pitch_prev, error_iterm_pitch_prev);
			error_rate_pitch_prev = PID_Return[0];
			error_iterm_pitch_prev = PID_Return[1];

			inputRoll = pid_update(error_rate_roll, P_Roll, I_Roll, D_Roll, error_rate_roll_prev, error_iterm_roll_prev);
			error_rate_roll_prev = PID_Return[0];
			error_iterm_roll_prev = PID_Return[1];
			
			inputYaw = pid_update(error_rate_yaw, P_Yaw, I_Yaw, D_Yaw, error_rate_yaw_prev, error_iterm_yaw_prev);
